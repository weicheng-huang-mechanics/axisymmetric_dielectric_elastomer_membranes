...
void elasticPlate::setupMass()
{
	...
	for (int i = 0; i < edgeNum; i++)
	{
		...
		double deltaV = M_PI * localRadius * localRadius * (v_edgeElement[i].x_2_start(1) - v_edgeElement[i].x_1_start(1));

		totalVolume = totalVolume + deltaV;
	}
}
...
void elasticPlate::setupGeometry()
{
    ...
    for (int i = 0; i < nv; i++)
    {
    	...
    	xCurrent(0) = rLocal * cos(deltaTheta * i) + 10.0;
    	xCurrent(1) = rLocal * sin(deltaTheta * i);
